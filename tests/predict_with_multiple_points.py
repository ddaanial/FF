...
if __name__ == '__main__':
    ...
    # Extract first frame
    cap = cv2.VideoCapture("test_10.mp4")
    ret, frame = cap.read()
    cap.release()
    ...
    def update_mask():
        if points:
            results = model(source=temp_image_path, points=points, labels=labels)
            masks_np = results[0].masks.data.cpu().numpy()
            if masks_np.shape[0] > 1:
                mask = np.any(masks_np, axis=0)  # Combine multiple masks
            else:
                mask = masks_np[0]
            mask = (mask > 0.5).astype(bool)  # Threshold if probability map
        ...
        cv2.waitKey(1)  # Ensure display updates
    ...
    def mouse_callback(event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            labels.append(1)
            print(f"🖱️ Selected positive point: ({orig_x}, {orig_y})")
            update_mask()
            ...
            points.append([orig_x, orig_y])
            labels.append(0)
            print(f"🖱️ Selected negative point: ({orig_x}, {orig_y})")
            update_mask()
    ...
    results = predictor(source="test_10.mp4", points=[points], labels=[labels])
