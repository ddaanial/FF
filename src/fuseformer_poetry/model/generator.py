...
class InpaintGenerator(BaseNetwork):
    def __init__(self, init_weights=True):
        ...
        blocks = []
        dropout = 0.
        t2t_params = {'kernel_size': kernel_size, 'stride': stride,
                      'padding': padding, 'output_size': output_size}
        n_vecs = 1
        for i, d in enumerate(kernel_size):
            n_vecs *= int((output_size[i] + 2 *
                          padding[i] - (d - 1) - 1) / stride[i] + 1)
        for _ in range(stack_num):
            blocks.append(TransformerBlock(hidden=hidden, num_head=num_head, dropout=dropout, n_vecs=n_vecs,
                                           t2t_params=t2t_params))
        self.transformer = nn.Sequential(*blocks)
        self.ss = SoftSplit(channel // 2, hidden, kernel_size,
                            stride, padding, dropout=dropout)
        self.add_pos_emb = AddPosEmb(n_vecs, hidden)
        self.sc = SoftComp(channel // 2, hidden, output_size,
...
class AddPosEmb(nn.Module):
    def __init__(self, n, c):
        ...
        self.pos_emb = nn.Parameter(torch.zeros(1, 1, n, c).float().normal_(
            mean=0, std=0.02), requires_grad=True)
        self.num_vecs = n
...
class SoftComp(nn.Module):
    def __init__(self, channel, hidden, output_size, kernel_size, stride, padding):
        ...
        self.bias = nn.Parameter(torch.zeros(
            (channel, h, w), dtype=torch.float32), requires_grad=True)
...
class FusionFeedForward(nn.Module):
    ...
    def forward(self, x):
        x = self.conv1(x)
        b, n, c = x.size()
        normalizer = x.new_ones(
            b, n, 49).view(-1, self.n_vecs, 49).permute(0, 2, 1)
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, hidden=128, num_head=4, dropout=0.1, n_vecs=None, t2t_params=None):
        ...
        self.attention = MultiHeadedAttention(
            d_model=hidden, head=num_head, p=dropout)
